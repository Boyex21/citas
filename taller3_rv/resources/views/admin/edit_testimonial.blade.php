                        ...
                        <form action="{{ route('admin.testimonial.update', $testimonial->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-12">
                                    ...
                                    <textarea name="comment" id="comment" cols="30" rows="10" class="form-control text-area-5">{{ filterTag(clean($testimonial->comment)) }}</textarea>
                                </div>

                                <div class="form-group col-12">
                                    <label>{{__('admin.Show Homepage')}} <span class="text-danger">*</span></label>
                                    <select name="show_homepage" class="form-control">
                                        <option {{ $testimonial->show_homepage == 1 ? 'selected' : '' }} value="1">{{__('admin.Enable')}}</option>
                                        <option {{ $testimonial->show_homepage == 0 ? 'selected' : '' }} value="0">{{__('admin.Disable')}}</option>
                                    </select>
                                </div>
