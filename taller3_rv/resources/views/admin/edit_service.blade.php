                        ...
                        <form action="{{ route('admin.service.update',$service->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-12">
                                    ...
                                    <textarea name="short_description" cols="30" rows="10" class="form-control text-area-5">{{ filterTag(($service->short_description)) }}</textarea>
                                </div>

                                <div class="form-group col-12">
                                    <label>{{__('admin.Description')}} <span class="text-danger">*</span></label>
                                    <textarea id="summernote" name="description" cols="30" rows="10" class="summernote">{!! clean(($service->description)) !!}</textarea>
                                </div>

                                <div class="form-group col-12">
                                    <label>{{__('admin.Show Homepage')}} <span class="text-danger">*</span></label>
                                    <select name="show_homepage" class="form-control">
                                        <option {{ $service->show_homepage == 1 ? 'selected' : '' }} value="1">{{__('admin.Enable')}}</option>
                                        <option {{ $service->show_homepage == 0 ? 'selected' : '' }} value="0">{{__('admin.Disable')}}</option>
                                    </select>
                                </div>
                                ...
                                <div class="form-group col-12">
                                    ...
                                    <textarea name="seo_description" id="" cols="30" rows="10" class="form-control text-area-5">{{ filterTag(clean($service->seo_description)) }}</textarea>
                                </div>
